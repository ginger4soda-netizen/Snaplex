...
const ChatBot: React.FC<Props> = ({ messages, onUpdateMessages, imageContext, systemLanguage, settings }) => {
  ...
  useEffect(() => {
      if (!initialized.current && messages.length === 0) {
          ...
          const greetingText = systemLanguage === 'Chinese' 
            ? "对这张图你还有什么想了解的？"
            : "What else do you want to know about this image?";
  ...
  }, [messages, onUpdateMessages, systemLanguage]);
    ...
    <div className="flex flex-col h-full bg-cream animate-[fadeIn_0.3s_ease-out] rounded-t-[2.5rem] overflow-hidden">
      <div className="flex-1 overflow-y-auto p-6 space-y-4 pb-20">
        {messages.map((msg, index) => (
          ...
          >
            <div 
              className={`relative max-w-[85%] p-4 rounded-2xl shadow-sm text-lg leading-relaxed group ${
                msg.role === 'user' 
                  ? 'bg-stone-800 text-white rounded-tr-none' 
                  : 'bg-white text-stone-800 rounded-tl-none border border-stone-100 pb-9' // Added padding bottom for copy button
              }`}
            >
               {msg.text}
               ...
               {/* Copy Button for Model messages (Excluding the first greeting message) */}
               {msg.role === 'model' && msg.text && index > 0 && (
                   <button
                       onClick={() => handleCopy(msg.text, msg.id)}
                       className="absolute bottom-2 right-2 p-1.5 text-coral/70 hover:text-coral transition-colors rounded-lg hover:bg-coral/10 active:scale-95"
                       title="Copy answer"
                   >
                       {copiedId === msg.id ? (
      ...
      <div className="p-4 bg-white/80 backdrop-blur border-t border-stone-100 absolute bottom-0 left-0 right-0">
        <div className="flex items-center gap-2 bg-white p-2 rounded-full shadow-lg border border-stone-100">
          <input 
            type="text" 
            className="flex-1 bg-transparent px-4 py-2 outline-none text-stone-800 placeholder-stone-400"
            placeholder={systemLanguage === 'Chinese' ? "输入你的问题..." : "Ask something..."}
            value={input}
          ...
          <button 
            onClick={handleSend}
            disabled={loading}
            className="w-10 h-10 bg-coral text-white rounded-full flex items-center justify-center shadow-sm active:scale-95 transition-transform disabled:opacity-50"
          >
             {loading ? (
