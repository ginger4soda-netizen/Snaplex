...
}> = ({ title, systemLabel, content, color, isGlobalFlipped }) => {
    ...
    return (
        <div className="relative mb-6 perspective-1000">
             <div className="flex justify-between items-center mb-2 px-5">
                <div className={`text-xs font-black uppercase tracking-widest ${color}`}>
                    {systemLabel}
                </div>
                <button 
                    onClick={handleCopy} 
                    className="p-1.5 rounded-lg transition-colors text-stone-300 hover:bg-stone-100"
                    style={{ color: isCopied ? hexColor : undefined }}
                    onMouseEnter={(e) => e.currentTarget.style.color = hexColor}
                    onMouseLeave={(e) => e.currentTarget.style.color = isCopied ? hexColor : ''}
                    title="Copy"
                >
            ...
            <div className={`relative z-10 bg-white rounded-2xl shadow-sm transition-all duration-500 transform style-preserve-3d ${isGlobalFlipped ? 'rotate-y-180' : ''}`}>
                <div className={`p-5 flex flex-col backface-hidden ${isGlobalFlipped ? 'hidden' : 'block'}`}>
                    <div className="leading-relaxed text-stone-700 font-medium text-lg mb-1 relative">
                        {content.original}
                    </div>
                </div>
                <div className={`inset-0 bg-stone-50 rounded-2xl p-5 flex flex-col rotate-y-180 backface-hidden ${isGlobalFlipped ? 'block' : 'hidden'}`}>
                    <div className="leading-relaxed text-stone-800 font-medium text-lg mb-1">
                        {content.translated}
                    </div>
                </div>
            </div>
...
const AnalysisView: React.FC<Props> = ({ 
    image, analysis, onBack, settings, 
  ...
  const modules = hasStructuredPrompts ? [
      { title: 'Subject', label: settings.systemLanguage === 'Chinese' ? '画面主体' : 'SUBJECT', color: 'text-coral', content: analysis.structuredPrompts?.subject },
      { title: 'Environment', label: settings.systemLanguage === 'Chinese' ? '空间环境' : 'ENVIRONMENT', color: 'text-mint', content: analysis.structuredPrompts?.environment },
      { title: 'Composition', label: settings.systemLanguage === 'Chinese' ? '构图/视角' : 'COMPOSITION', color: 'text-softblue', content: analysis.structuredPrompts?.composition },
      { title: 'Lighting/Color', label: settings.systemLanguage === 'Chinese' ? '光影色调' : 'LIGHTING/COLOR', color: 'text-sunny', content: analysis.structuredPrompts?.lighting },
      { title: 'Mood', label: settings.systemLanguage === 'Chinese' ? '情绪氛围' : 'MOOD', color: 'text-softblue', content: analysis.structuredPrompts?.mood },
      { title: 'Style', label: settings.systemLanguage === 'Chinese' ? '美术风格' : 'STYLE', color: 'text-stone-500', content: analysis.structuredPrompts?.style },  
  ] : [
      { title: 'Description', label: settings.systemLanguage === 'Chinese' ? '描述' : 'DESCRIPTION', color: 'text-softblue', content: { original: analysis.description, translated: "Translation unavailable." } }
  ];
  ...
  const handleGlobalCopy = () => {
      ...
      const text = modules
        .filter(m => allowed.includes(m.title)) // Check if module title is in allowed list
        .map(m => {
      ...
      alert(isGlobalFlipped ? "Copied configured translated prompts!" : "Copied configured prompts!");
  };
  ...
  const handleDownloadImage = () => {
      ...
      link.href = image;
      link.download = `snaplex-${Date.now()}.png`;
    ...
    <div className="pb-0 bg-cream min-h-screen flex flex-col">
      ...
      {isZoomed && (
          <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-6 animate-[fadeIn_0.2s]" onClick={() => setIsZoomed(false)}>
              <div className="relative max-w-full max-h-[90vh]" onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={() => setIsZoomed(false)} className="absolute -top-12 right-0 text-white hover:text-coral transition-colors">
      ...
      <div className="relative group shrink-0">
        ...
        <div className="absolute top-4 left-4 right-4 flex justify-between items-center z-10">
            <button onClick={onBack} className="w-10 h-10 bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-black/50 transition-colors border border-white/10">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            </button>
            <div className="flex gap-2">
                 <button onClick={handleCopyImage} className="w-10 h-10 bg-black/30 backdrop-blur-md text-white rounded-full flex items-center justify-center hover:bg-black/50 transition-colors border border-white/10" title="Copy Image">
                   <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                </button>
                <button onClick={() => setIsZoomed(true)} className="w-10 h-10 bg-black/30 backdrop-blur-md text-white rounded-full flex items-center justify-center hover:bg-black/50 transition-colors border border-white/10" title="Zoom">
                ...
                <button onClick={handleDownloadImage} className="w-10 h-10 bg-black/30 backdrop-blur-md text-white rounded-full flex items-center justify-center hover:bg-black/50 transition-colors border border-white/10" title="Download Image">
      ...
      <div className="flex-1 bg-cream rounded-t-[2.5rem] -mt-8 relative z-20 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] overflow-hidden flex flex-col">
        <div className={`flex-1 flex flex-col h-full relative ${showChat ? 'flex' : 'hidden'}`}>
             <button onClick={() => setShowChat(false)} className="absolute top-4 right-6 z-30 text-stone-400 hover:text-stone-800 p-2 bg-white/50 rounded-full">
        ...
        <div className={`px-6 py-8 flex-1 ${showChat ? 'hidden' : 'block'}`}>
                ...
                <button onClick={() => setShowChat(true)} className="fixed bottom-6 right-6 w-14 h-14 bg-stone-800 text-white rounded-full flex items-center justify-center shadow-2xl hover:scale-110 active:scale-95 transition-all z-40 border-2 border-cream">
