...
### 现有 AI Provider 接口（来自 `services/providers/types.ts`）
...
| CLIP 集成 | `ort` (ONNX Runtime) crate 加载 `clip-vit-b-32-int8.onnx`，导入时自动推理 |
...
gantt
    ...
    用户测试 Phase 0       :u0, after a0, 1
    用户测试 Phase 1       :u1, after b1, 2
    用户测试 Phase 2       :u2, after c1, 2
    用户测试 Phase 3       :u3, after d0, 2
    
    section Agent E (Landing)
    Landing Page (最后)     :e0, after u3, 7
```
...
| **E: Landing Page** | 网页端首页重构 + 路由拆分 | `landing/` 或 `src/pages/Landing.tsx` | **最后启动**（功能和设计稳定后） |
...
| **共享文件** | `types.ts`, `package.json`, `App.tsx` 为共享文件，修改前需协调 |
...
## 10. 协调 Agent（Coordinator）

> 在 Antigravity 终端中开启一个 Claude Code 主实例作为 Coordinator Agent。

### 10.1 Coordinator 职责

| 职责 | 说明 |
|------|------|
| **任务分配** | 读取本文档 §8，将各 Agent Prompt 分配给独立的 Claude Code 终端实例 |
| **依赖管理** | 确认 Agent A 完成 Phase 0 后再启动 Agent B/C |
| **接口验证** | Agent A 完成后验证所有 IPC Commands 是否与 §5 一致 |
| **集成合并** | 按 §8.5 合并顺序（A → B → C → D → E）合并各分支 |
| **冲突解决** | 处理共享文件（`types.ts`, `package.json`, `App.tsx`）的合并冲突 |
| **用户测试协调** | 在每个 Phase 完成后通知用户测试，收集反馈分发给对应 Agent |

### 10.2 Coordinator Prompt

```
你是 Snaplex 桌面应用重构项目的 Coordinator Agent。

## 你的职责
你不写代码，你负责协调多个 Claude Code 实例并行开发。

## 关键文档
- 完整规范：docs/AGENT_SPEC.md
- 用户可读计划：docs/DEVELOPMENT_PLAN.md

## 工作流程
1. 读取 docs/AGENT_SPEC.md，理解全部 6 个 Phase + 5 个 Agent
2. 启动 Agent A（Tauri Backend），将 §8.3 Agent A Prompt 发送到新终端
3. 等待 Agent A 完成 Phase 0，验证 IPC Commands 签名
4. 并行启动 Agent B（UI）和 Agent C（Search+Color）
5. 在每个 Phase 完成后通知用户运行 `pnpm tauri dev` 测试
6. 收集用户反馈，分发给对应 Agent 修改
7. Agent A HTTP Server 完成后启动 Agent D（Extension）
8. 所有功能完成 + 用户满意后，最后启动 Agent E（Landing Page）

## 合并流程
每个 Agent 在独立 Git 分支工作：
- feat/agent-a-backend
- feat/agent-b-ui
- feat/agent-c-search
- feat/agent-d-extension
- feat/agent-e-landing

合并顺序：A → B → C → D → E
每次合并后运行 pnpm tauri dev 验证。

## 用户测试检查点
在以下时机暂停并通知用户测试：
- Phase 0 完成：验证窗口启动、图片导入
- Phase 1 完成：验证三栏布局、文件夹、右键菜单、主题
- Phase 2 完成：验证搜索、色卡
- Phase 3 完成：验证浏览器插件
- Phase 4 完成：验证全面测试结果
- Phase 5 完成：验证 Landing Page
```
...
这会启动一个带有 Tauri 窗口的开发版应用，代码修改后自动热重载。

#### 各阶段测试要点

| Phase | 你需要测试什么 | 怎么测 |
|-------|--------------|--------|
| **0** | 应用能启动、图片能导入、Finder 中能看到 .snpx 文件夹 | 运行 `pnpm tauri dev`，拖入几张图片 |
| **1** | 三栏布局、文件夹操作、拖拽上传、右键菜单、主题切换、按需分析 | 在 App 中操作所有 UI 交互 |
| **2** | 搜索各种关键词、色卡是否准确、复制色码 | 导入 10+ 张图后尝试各种搜索 |
| **3** | Chrome 插件悬浮 Logo、点击保存、视频截图、Eagle 导入 | Chrome 加载未打包插件测试 |
| **4** | 跑完全部测试 + 完整使用流程 | Agent 运行测试，你做端到端体验 |
| **5** | Landing Page 视觉效果和下载链接 | 浏览器打开 localhost 查看 |

#### 反馈方式

直接在 Coordinator Agent 的终端中输入反馈，例如：
- "右栏色卡太宽了，缩小到 50% 宽度"
- "搜索'动画'没有返回结果，但有3张动画图片"
- "Dark Mode 的文件夹名看不清"

Coordinator 会将反馈分发给对应的 Agent 修改。

#### 什么时候需要安装正式版？

只有在 **Phase 4（测试）全部通过** 后，你才需要运行 `pnpm tauri build` 生成安装包（`.dmg`），安装到系统中做最终体验测试。日常开发全程用 `pnpm tauri dev` 即可。
